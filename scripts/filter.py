...
def generate_header(hb_coeffs, N):
    """ Generate the Q1.15 array of coefficients in a header file """

    num_coeffs = len(hb_coeffs)
    x = 0
    with open("emon_CM_coeffs.h", "w") as f_out:
        f_out.write("/* AUTO GENERATED by scripts/filter.py */\n\n")

        f_out.write("#include <stdint.h>\n\n")
        f_out.write(f"#define DOWNSAMPLE_TAPS {N+1}u\n\n")
        f_out.write("static const unsigned int  downsample_taps = DOWNSAMPLE_TAPS;\n")
        f_out.write(f"static const unsigned int  numCoeffUnique  = {num_coeffs}u;\n")
