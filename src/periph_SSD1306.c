...
#define SSD1306_COMMAND_STREAM      0x00u   /* Continuation bit=0, D/C=0; 0000 0000 */
...
#define SSD1306_SET_MUX_RATIO       0xA8u   /* Set MUX ratio to N+1 MUX, N=A[5:0] : from 16MUX to 64MUX */
...
#define SSD1306_DIS_ENT_DISP_ON     0xA4u   /* Entire Display ON, Output ignores RAM content */
#define SSD1306_DIS_IGNORE_RAM      0xA5u   /* Resume to RAM content display, Output follows RAM content */
#define SSD1306_DIS_NORMAL          0xA6u   /* Normal display, 0 in RAM: OFF in display panel, 1 in RAM: ON in display panel */
#define SSD1306_DIS_INVERSE         0xA7u   /* Inverse display, 0 in RAM: ON in display panel, 1 in RAM: OFF in display panel */
#define SSD1306_DEACT_SCROLL        0x2Eu   /* Stop scrolling that is configured by command 26h/27h/29h/2Ah */
#define SSD1306_ACTIVE_SCROLL       0x2Fu   /* Start scrolling that is configured by the scrolling setup commands:26h/27h/29h/2Ah */
...
#define SSD1306_SEG_REMAP           0xA0u   /* Set Segment Re-map, X[0]=0b column address 0 is mapped to SEG0 */
#define SSD1306_SEG_REMAP_OP        0xA1u   /* Set Segment Re-map, X[0]=1b: column address 127 is mapped to SEG0 */
#define SSD1306_COM_SCAN_DIR        0xC0u   /* Set COM Output, X[3]=0b: normal mode (RESET) Scan from COM0 to COM[N –1], e N is the Multiplex ratio */
#define SSD1306_COM_SCAN_DIR_OP     0xC8u   /* Set COM Output, X[3]=1b: remapped mode. Scan from COM[N-1] to COM0, e N is the Multiplex ratio */
#define SSD1306_COM_PIN_CONF        0xDAu   /* Set COM Pins Hardware Configuration,
...
#define SSD1306_SET_CONTRAST        0x81u   /* Set Contrast Control, Double byte command to select 1 to 256 contrast steps, increases as the value increases */
#define SSD1306_SET_OSC_FREQ        0xD5u   /* Set Display Clock Divide Ratio/Oscillator Frequency
...
#define SSD1306_SET_CHAR_REG        0x8Du   /* Charge Pump Setting, A[2] = 0b, Disable charge pump(RESET), A[2] = 1b, Enable charge pump during display on
...
#define SSD1306_RESET               0xE4u   /* Maybe SW RESET, @source https://github.com/SmingHub/Sming/issues/501 */
...
static SSD1306_Status_t
drawChar(const char c)
{
    unsigned int i = 0;
    SSD1306_Status_t update = bufUpdatePos();
    if (FAIL_UPDATE_POS == update)
    {
        return FAIL_UPDATE_POS;
    }
...
ssd1306DrawString(const char *s)
{
    ...
    {
        ret = drawChar(*s++);
        if (FAIL_UPDATE_POS == ret)
        {
            return FAIL_DRAW_STRING;
        }
    }

    return SUCCESS_DRAW_STRING;
}
...
ssd1306Init(Sercom *pSercomI2C)
{
    const uint8_t initCmds[SSD1306_NUM_INIT_CMDS] = {
        ...
        SSD1306_SET_MUX_RATIO, 0x3Fu,       /* 0x1F -> 128x32, 0x3F -> 128x64 */
        SSD1306_DISPLAY_OFFSET, 0x00u,
        SSD1306_SET_START_LINE,
        SSD1306_SET_CHAR_REG, 0x14u,
        SSD1306_MEMORY_ADDR_MODE, 0x00u,    /* 0x00 -> horizontal, 0x01 -> vertical
                                             * 0x02 -> page
                                             */
        SSD1306_SEG_REMAP_OP,
        SSD1306_COM_SCAN_DIR_OP,
        SSD1306_COM_PIN_CONF, 0x12u,        /* 0x02 -> 128x32, 0x12 -> 128x64 */
        SSD1306_SET_CONTRAST, 0x7Fu,
        SSD1306_SET_PRECHARGE, 0xC2u,
        SSD1306_VCOM_DESELECT, 0x20u,
...
void
ssd1306SetPosition(const PosXY_t pos)
{
    posBuf = pos.x + (pos.y << 7u);
}
